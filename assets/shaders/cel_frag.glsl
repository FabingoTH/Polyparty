...
float shadowCalc() {
    vec3 pos = vertexData.fragPosLight.xyz * 0.5 + 0.5;
...
vec3 shade(vec3 unitNormal, vec3 unitToLight, vec3 unitToCamera, vec3 diffuse, vec3 specc, float shininess)
{
    ...
    float celSpec = pow(HdotN, shininess);
    float celLevelSpec = floor(celSpec * CEL_LEVELS);
    celSpec = celLevelSpec / CEL_LEVELS;

    float shadow = shadowCalc();

    return shadow * (diffuse * brightness + specc * celSpec);
}
